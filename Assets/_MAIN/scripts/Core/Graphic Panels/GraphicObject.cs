...
public class GraphicObject
{
    ...
    private Material GetTransitionalMaterial()
    {
    ...
    public IEnumerator Fading(float target, float speed, Texture blend)
    {
        ...
        string opacityParameter = isBlending ? MATERIAL_FIELD_BLEND : MATERIAL_FIELD_ALPHA;

        while (renderer.material.GetFloat(opacityParameter) != target)
        {
            float opacity = Mathf.MoveTowards(renderer.material.GetFloat(opacityParameter), target, speed * GraphicPanelManager.defaultTransitionSpeed * Time.deltaTime);
            renderer.material.SetFloat(opacityParameter, opacity);
