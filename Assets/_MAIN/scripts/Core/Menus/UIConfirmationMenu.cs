...
{
    public static UIConfirmationMenu instance;
    ...
    public void Show(string title, params ConfirmationButton[] options)
    {
        if (options.Length == 0)
        {
            Debug.LogError("Confirmation must have at least one option for the user to select.");
            return;
        }
    ...
    public struct ConfirmationButton
    {
        ...
        public bool autoCloseOnChoice;

        public ConfirmationButton(string title, Action action, bool autoCloseOnChoice = true)
        {
            this.title = title;
            this.action = action;
            this.autoCloseOnChoice = autoCloseOnChoice;
        }
    }
}
